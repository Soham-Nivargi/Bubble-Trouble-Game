...
public:
    ...
    void nextStep(double t)
    {
        ...
        if(vy>=0){
            ...
            new_y = circle.getY() + vy*t+0.5*acc*t*t;
            vy=vy+acc*t;//if going down velocity increases downwards
            }
        else {
              new_y=circle.getY() + vy*t-0.5*acc*t*t;
              vy=vy-acc*t;//if going up velocity increases upwards
              }
        ...
        if(vy<0 && new_y < (0 + circle.getRadius())){
            vy=-vy;
            new_y=circle.getY() + vy*t + 0.5*acc*t*t;//bounce along Y direction at upper border
            }
        else if(vy > 0 && new_y > (450 - circle.getRadius())){
            vy=-vy;
            new_y=circle.getY() + vy*t - 0.5*acc*t*t;//bounce along Y direction at lower border
            }
    ...
    void update_pos(){
        circle.moveTo(250,100);//respawn bubble after hitting shooter
        return;
        }
